...
{% endhighlight %}
&emsp;&emsp;要分析sinkAndSubscription的类型要追终到Observable的run方法中。在前面分析RxSwift的事件响应流程中已经分析过sink的run方法的执行本质上就是执行创建序列的闭包，而在文章的最前面提到了执行创建序列的闭包会返回一个AnonymousDisposable（create）对象。所以sinkAndSubscription对象持有的两个属性就是这里的AnonymousObservableSink对象和AnonymousDisposable（create）对象。
